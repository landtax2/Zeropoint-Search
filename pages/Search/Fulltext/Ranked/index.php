...
if (strlen($fulltext) > 1) {
    ...
    $where[] = "t2.full_text @@ to_tsquery('english', :fulltext)";
...
if (count($where) > 0) {
    $queryText = "
    SELECT t1.id, t1.name, t1.path, t1.ai_title, t2.full_text, t1.ai_summary, t1.last_found, t1.date_created, t1.date_modified, t1.ai_tags, t1.ai_contact_information,
    ts_rank(to_tsvector('english', t2.full_text), to_tsquery('english', :fulltext)) AS rank
    FROM network_file t1
    LEFT JOIN network_file_fulltext t2 ON t1.id = t2.network_file_id
    WHERE $where_s
    AND t1.found_last = 1
    ORDER BY rank DESC
    LIMIT 200";
...
$common->print_template_card('Ranked Fulltext Search', 'start');
?>
...
<?php
//if fulltext is not stored, show a warning
if ($common->get_config_value('STORE_FULLTEXT') == "0") {
    echo '<div class="alert alert-warning" role="alert">
        <i class="fa fa-exclamation-triangle me-2"></i>
        <strong>Warning:</strong> The STORE_FULLTEXT configuration is set to 0. There may not be any fulltext content available to search unless it was indexed before this setting was changed.
    </div>';
}
?>
...
<p>Description: Used to search fulltext based on a ranked algorithm. Search limited to 200 results.</p>
<div class="row mb-3">
    <div class="col-md-6">
        <div class="form-group">
            <label for="ai_summary">Fulltext: <span class="text-danger">*</span></label>
            <input class="form-control" placeholder="Words that are included in the fulltext, separated by comma or space" type="text" value="<?= isset($_GET['fulltext']) ? $_GET['fulltext'] : ''; ?>" id="fulltext" onkeypress="if(event.keyCode==13){search(); }" required>
